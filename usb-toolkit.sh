...
cleanup() {
    ...
    if [[ -n "$CLEANUP_TMPFILE" && -f "$CLEANUP_TMPFILE" ]]; then
        rm -f "$CLEANUP_TMPFILE" 2>/dev/null
        log "CLEANUP: Removed incomplete file ${CLEANUP_TMPFILE}"
    fi
    # Unmount temp mounts
    if [[ -n "$CLEANUP_MOUNT" ]] && findmnt -rn "$CLEANUP_MOUNT" &>/dev/null; then
        sync
        umount "$CLEANUP_MOUNT" 2>/dev/null || true
        rmdir "$CLEANUP_MOUNT" 2>/dev/null || true
        log "CLEANUP: Unmounted ${CLEANUP_MOUNT}"
    fi
    # Release lockfile
    release_lock
...
acquire_lock() {
    ...
    echo -e "\n${RED}${BOLD}  ERROR: Another instance is running (PID: ${lock_pid}).${NC}"
...
validate_filepath() {
    ...
    # Expand tilde (SC2088: intentional literal match, not expansion)
    local tilde="~"
    if [[ "$path" == "${tilde}/"* ]]; then
        local user
...
dd_with_progress() {
    ...
    if command -v pv &>/dev/null; then
        ...
        dd if="$input" bs="$bs" "${extra_args[@]}" 2>/dev/null | pv "${pv_args[@]}" | dd of="$output" bs="$bs" 2>/dev/null
        return "${PIPESTATUS[0]}"
    else
...
double_confirm() {
    ...
    local title_pad_total=$((box_width - ${#title} - 2))
    ...
    printf "  ${RED}${BOLD}  ║%*s${title}%*s║${NC}\n" "$title_pad_left" "" "$title_pad_right" ""
    printf "  ${RED}${BOLD}  ║%*s${inner_text}%*s║${NC}\n" "$pad_left" "" "$pad_right" ""
    echo -e "  ${RED}${BOLD}  ╚${border_inner}╝${NC}"
    echo ""
...
is_system_disk() {
    local dev="$1"
    # Strip partition number to get base device (handles both sd and NVMe)
    local base_dev
    base_dev=$(echo "$dev" | sed -e 's/p[0-9]*$//' -e 's/[0-9]*$//')

    # Check if root filesystem is on this device
    local root_dev
    root_dev=$(findmnt -rno SOURCE / 2>/dev/null | sed -e 's/p[0-9]*$//' -e 's/[0-9]*$//' -e 's|/dev/||')
...
print_device_details() {
    ...
    vendor_id=$(read_sysfs "/sys/block/${dev}/device/../../idVendor" "????")
    product_id=$(read_sysfs "/sys/block/${dev}/device/../../idProduct" "????")
...
unmount_usb() {
    ...
    case "$mode" in
        ...
        3)
            ...
            sync
            if umount -l "/dev/${dev}" 2>/dev/null; then
...
format_usb() {
    ...
    if "${mkfs_args[@]}" "/dev/${part_dev}" &>/dev/null; then
...
health_read_speed_test() {
    ...
    local speed
    speed=$(echo "$dd_output" | grep -oP '[\d.,]+ [MGKT]?B/s' | tail -1)
    if [[ -n "$speed" ]]; then
...
backup_to_image() {
    ...
    echo ""
    local default_img
    default_img="/home/$(get_current_user)/usb-backup-${dev}-$(date +%Y%m%d-%H%M%S)${ext}"
    ...
    case "$compress" in
        none)
            ...
            ;;
        gzip)
            if dd if="/dev/${dev}" bs=4M 2>/dev/null | pv -s "$size_bytes" 2>/dev/null | gzip -c > "$img_file"; then
                backup_ok=1
            elif dd if="/dev/${dev}" bs=4M status=progress 2>&1 | gzip -c > "$img_file"; then
                backup_ok=1
            fi
            ;;
        zstd)
            if dd if="/dev/${dev}" bs=4M 2>/dev/null | pv -s "$size_bytes" 2>/dev/null | zstd -c > "$img_file"; then
                backup_ok=1
            elif dd if="/dev/${dev}" bs=4M status=progress 2>&1 | zstd -c > "$img_file"; then
                backup_ok=1
            fi
            ;;
    esac
...
restore_from_image() {
    ...
    echo ""
    read -rp "  Path to image file: " img_file_raw
...
write_iso() {
    ...
    echo ""
    read -rp "  Path to ISO file: " iso_file_raw
    ...
    if [[ "$verify" == "1" || -z "$verify" ]]; then
        ...
        local usb_sha
        usb_sha=$(dd if="/dev/${dev}" bs=4M count=$((iso_bytes / 4194304 + 1)) 2>/dev/null | head -c "$iso_bytes" | sha256sum | cut -d' ' -f1)
        echo -e "    USB:    ${usb_sha}"
...
wipe_random() {
    ...
    dd_with_progress "/dev/urandom" "/dev/${dev}" "4M" || true
    sync
...
cli_help() {
    cat <<'HELPEOF'
USB TOOLKIT v2.0 — USB Device Operations & Management
